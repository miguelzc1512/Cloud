...
var isBinaryPath = (filePath) => binaryExtensions.has(node_path.extname(filePath).slice(1).toLowerCase());
var foreach = (val, fn) => {
...
function createFsWatchInstance(path, options, listener, errHandler, emitRaw) {
	const handleEvent = (rawEvent, evPath) => {
		...
		if (evPath && path !== evPath) fsWatchBroadcast(node_path.resolve(path, evPath), KEY_LISTENERS, node_path.join(path, evPath));
	};
	try {
...
var NodeFsHandler = class {
	...
	*/
	_handleFile(file, stats, initialAdd) {
		...
		const basename = node_path.basename(file);
	...
	*/
	...
	_handleRead(directory, initialAdd, wh, target, dir, depth, throttler) {
		directory = node_path.join(directory, "");
		...
		stream.on(STR_DATA, async (entry) => {
			...
			let path = node_path.join(directory, item);
			current.add(item);
			...
			if (item === target || !target && !previous.has(item)) {
				this.fsw._incrReadyCount();
				path = node_path.join(dir, node_path.relative(dir, path));
	...
	*/
	async _handleDir(dir, stats, initialAdd, depth, target, wh, realpath) {
		const parentDir = this.fsw._getWatchedDir(node_path.dirname(dir));
		const tracked = parentDir.has(node_path.basename(dir));
		if (!(initialAdd && this.fsw.options.ignoreInitial) && !target && !tracked) this.fsw._emit(EV.ADD_DIR, dir, stats);
		parentDir.add(node_path.basename(dir));
	...
	async _addToNodeFs(path, initialAdd, priorWh, depth, target) {
		...
		try {
			...
			if (stats.isDirectory()) {
				...
				const targetPath = follow ? await (0, node_fs_promises.realpath)(path) : path;
				if (this.fsw.closed) return;
				const parent = node_path.dirname(wh.watchPath);
				this.fsw._getWatchedDir(parent).add(wh.watchPath);
				this.fsw._emit(EV.ADD, wh.watchPath, stats);
				closer = await this._handleDir(parent, stats, initialAdd, depth, path, wh, targetPath);
				if (this.fsw.closed) return;
				if (targetPath !== void 0) this.fsw._symlinkPaths.set(node_path.resolve(path), targetPath);
...
function createPattern(matcher) {
	...
	if (typeof matcher === "object" && matcher !== null) return (string) => {
		...
		if (matcher.recursive) {
			const relative = node_path.relative(matcher.path, string);
			if (!relative) return false;
			return !relative.startsWith("..") && !node_path.isAbsolute(relative);
		}
		return false;
	};
	return () => false;
...
var normalizePathToUnix = (path) => toUnix(node_path.normalize(toUnix(path)));
var normalizeIgnored = (cwd = "") => (path) => {
	if (typeof path === "string") return normalizePathToUnix(node_path.isAbsolute(path) ? path : node_path.join(cwd, path));
	else return path;
};
var getAbsolutePath = (path, cwd) => {
	if (node_path.isAbsolute(path)) return path;
	return node_path.join(cwd, path);
};
...
var DirEntry = class {
	...
	async remove(item) {
		...
		try {
			...
			if (this._removeWatcher) this._removeWatcher(node_path.dirname(dir), node_path.basename(dir));
		}
	}
	has(item) {
		const { items } = this;
...
var WatchHelper = class {
	...
	constructor(path, follow, fsw) {
		...
		this.fullWatchPath = node_path.resolve(watchPath);
		this.dirParts = [];
	...
	entryPath(entry) {
		return node_path.join(this.watchPath, node_path.relative(this.watchPath, entry.fullPath));
	}
	filterPath(entry) {
		const { stats } = entry;
...
var FSWatcher = class extends node_events.EventEmitter {
	...
	add(paths_, _origAdd, _internal) {
		...
		Promise.all(paths.map(async (path) => {
			...
			results.forEach((item) => {
				if (item) this.add(node_path.dirname(item), node_path.basename(_origAdd || item));
			});
		});
		return this;
	}
	...
	*/
	unwatch(paths_) {
		...
		const { cwd } = this.options;
		paths.forEach((path) => {
			if (!node_path.isAbsolute(path) && !this._closers.has(path)) {
				if (cwd) path = node_path.join(cwd, path);
	...
	*/
	getWatched() {
		...
		this._watched.forEach((entry, dir) => {
			const index = (this.options.cwd ? node_path.relative(this.options.cwd, dir) : dir) || ONE_DOT;
	...
	*/
	async _emit(event, path, stats) {
		...
		if (isWindows) path = node_path.normalize(path);
		if (opts.cwd) path = node_path.relative(opts.cwd, path);
		const args = [path];
		...
		if (opts.alwaysStat && stats === void 0 && (event === EVENTS.ADD || event === EVENTS.ADD_DIR || event === EVENTS.CHANGE)) {
			const fullPath = opts.cwd ? node_path.join(opts.cwd, path) : path;
			let stats;
			try {
	...
	*/
	_awaitWriteFinish(path, threshold, event, awfEmit) {
		...
		let timeoutHandler;
		let fullPath = path;
		if (this.options.cwd && !node_path.isAbsolute(path)) fullPath = node_path.join(this.options.cwd, path);
	...
	*/
	_getWatchedDir(directory) {
		const dir = node_path.resolve(directory);
	...
	*/
	_remove(directory, item, isDirectory) {
		const path = node_path.join(directory, item);
		const fullPath = node_path.resolve(path);
		...
		let relPath = path;
		if (this.options.cwd) relPath = node_path.relative(this.options.cwd, path);
	...
	*/
	_closePath(path) {
		...
		this._getWatchedDir(dir).remove(node_path.basename(path));
	}
	/**
...
var configPath = path.join(electron.app.getPath("userData"), "sync-config.json");
var statePath = path.join(electron.app.getPath("userData"), "sync-state.json");
var config = {
...
if (fs.existsSync(statePath)) try {
	uploadedState = JSON.parse(fs.readFileSync(statePath, "utf8"));
} catch (e) {}
var saveConfig = () => fs.writeFileSync(configPath, JSON.stringify(config, null, 2));
var saveState = () => fs.writeFileSync(statePath, JSON.stringify(uploadedState));
var isSyncPaused = false;
var pendingUploads = [];
var watchers = {};
...
function startWatching(folderPath) {
	...
	const watcher = chokidar_default.watch(folderPath, {
		ignored: /(^|[\/\\])\../,
		persistent: true,
		ignoreInitial: false
	});
	watcher.on("add", async (filePath) => {
		...
		} else await uploadFile(filePath);
	});
...
electron.ipcMain.handle("set-server-url", (event, url) => {
	config.serverUrl = url;
	saveConfig();
	return config;
});
electron.ipcMain.handle("get-sync-state", () => {
	return {
		paused: isSyncPaused,
		pendingCount: pendingUploads.length,
		pendingFiles: pendingUploads
...
electron.ipcMain.handle("link-folder", (event, folderPath) => {
