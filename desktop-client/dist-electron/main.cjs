...
import { join, relative, resolve as resolve$1, sep } from "node:path";
...
var ReaddirpStream = class extends Readable {
	...
	async _formatEntry(dirent, path) {
		...
		try {
			const fullPath = resolve$1(join(path, basename));
			entry = {
...
var http_default = isHttpAdapterSupported && function httpAdapter(config) {
	return wrapAsync(async function dispatchHttpRequest(resolve$2, reject, onDone) {
		...
		if (protocol === "data:") {
			...
			return settle(resolve$2, reject, {
				data: convertedData,
				status: 200,
				statusText: "OK",
		...
		req = transport.request(options, function handleResponse(res) {
			...
			if (responseType === "stream") {
				...
				settle(resolve$2, reject, response);
			} else {
...
var configPath = path.join(app.getPath("userData"), "sync-config.json");
var statePath = path.join(app.getPath("userData"), "sync-state.json");
var config = {
...
function startWatching(folderPath) {
	...
	watcher.on("add", async (filePath) => {
		...
		if ([
			...
			".mov",
			".avi"
		].includes(ext) && !uploadedState[filePath]) await uploadFile(filePath);
	});
...
async function uploadFile(filePath) {
	...
	try {
		...
		form.append("lastModified", stats.mtimeMs.toString());
		await axios.post(`${config.serverUrl}/api/upload`, form, { headers: { ...form.getHeaders() } });
